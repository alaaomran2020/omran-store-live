// Integration trigger: product AI pipeline
...
async function getGoogleAccessToken(serviceAccount) {
  ...
  const payload = b64url(JSON.stringify({ iss: serviceAccount.client_email, scope: 'https://www.googleapis.com/auth/spreadsheets', aud: 'https://oauth2.googleapis.com/token', iat: now, exp: now + 3600 }));
...
async function sheetsBatchUpdateValues(accessToken, data) {
  ...
  const response = await fetch(url, { method: 'POST', headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ valueInputOption: 'RAW', data }) });
...
function mapRows(values) {
  ...
  return rows.map((row, idx) => { const item = { __row: idx + 2 }; headers.forEach((h, i) => { item[h] = row[i] ?? ''; }); return item; });
}
...
async function generateWithGemini(row) {
  ...
  const response = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: buildPrompt(row) }] }], generationConfig: { responseMimeType: 'application/json', temperature: 0.2 } }) });
  ...
  let parsed; try { parsed = JSON.parse(text); } catch { throw new Error(`Gemini returned invalid JSON for ${row.product_id}: ${text.slice(0, 300)}`); }
  for (const field of REQUIRED_AI_FIELDS) if (!(field in parsed)) parsed[field] = '';
  return parsed;
}

function normalize(value) { return value === null || value === undefined ? '' : String(value).trim(); }
function buildWrite(rowNumber, ai) { return { range: `'إدخال منتج جديد'!R${rowNumber}:AC${rowNumber}`, values: [[normalize(ai.ai_product_name), normalize(ai.ai_short_description), normalize(ai.ai_full_description), normalize(ai.ai_category), normalize(ai.ai_age_group), normalize(ai.ai_seo_title), normalize(ai.ai_meta_description), normalize(ai.ai_alt_text), normalize(ai.ai_whatsapp_message), normalize(ai.ai_social_caption), normalize(ai.ai_review_notes), new Date().toISOString()]] }; }

async function main() {
  const accessToken = await getGoogleAccessToken(parseServiceAccount());
  const rows = mapRows((await sheetsGet(accessToken, INTAKE_RANGE)).values);
  const candidates = rows.filter(shouldGenerate).slice(0, MAX_ROWS);
  if (!candidates.length) { console.log('No products require AI generation.'); return; }
  console.log(`Generating AI content for ${candidates.length} product(s) with ${GEMINI_MODEL}.`);
  const writes = [];
  for (const row of candidates) {
    try { writes.push(buildWrite(row.__row, await generateWithGemini(row))); console.log(`Prepared ${row.product_id}`); }
    catch (error) { console.error(`Skipped ${row.product_id}:`, error.message); }
  }
