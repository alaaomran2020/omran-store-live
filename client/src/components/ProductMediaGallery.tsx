...
export function ProductMediaGallery({ product }: { product: Product }) {
  ...
  /* Escape closes the gallery only — the modal stack keeps the product dialog
     underneath open because the lightbox sits on top of it. */
  useFocusTrap(lightboxRef, lightboxOpen, closeLightbox);
  ...
  return (
    <div>
      ...
      {images.length > 1 && (
        /* aria-label needs a landmark/group role to be exposed — a bare div drops it. */
        <div role="group" aria-label="صور المنتج" className="mt-3 flex touch-pan-x snap-x snap-mandatory gap-2 overflow-x-auto overscroll-x-contain pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {images.map((image, index) => (
            <button
              ...
              className={`h-16 w-16 shrink-0 snap-start overflow-hidden rounded-xl border-2 bg-white transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-blue ${
      ...
      {lightboxOpen && selected && (
        <div
          ...
          className="fixed inset-0 z-[70] flex touch-pan-y items-center justify-center bg-black/95 p-3 outline-none sm:p-6"
        ...
        >
          ...
          >
            ...
            <div className="absolute bottom-1 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-xs font-extrabold text-white ring-1 ring-white/20 backdrop-blur sm:text-sm">
              <span aria-live="polite">{selectedIndex + 1} / {images.length}</span>
