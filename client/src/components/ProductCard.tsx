...
export function ProductCard({
  ...
  const isPopupProduct = isPopUpProduct(product);
  ...
  return (
    <article
      ...
      data-catalog={isPopupProduct ? "popup" : "toys"}
    ...
    >
      <button
        ...
        className={`relative block w-full overflow-hidden bg-brand-cream text-right focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-blue/15 ${
          isPopupProduct ? "aspect-[4/3]" : "aspect-square"
        }`}
        aria-label={`عرض تفاصيل ${product.name}`}
      >
        <ProductImage
          product={product}
          className={`h-full w-full transition duration-300 sm:group-hover:scale-[1.02] ${
            isPopupProduct ? "object-contain p-2 sm:p-3" : "object-cover"
          }`}
      ...
      <div className={`flex flex-1 flex-col gap-2.5 ${isPopupProduct ? "p-3 sm:gap-2.5 sm:p-4" : "p-3 sm:gap-3 sm:p-5"}`}>
