...
export function ProductDetailsDialog({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex items-end justify-center bg-brand-navy/60 p-0 backdrop-blur-sm sm:items-center sm:p-6"
      onClick={onClose}
    >
      <div
        className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-t-2xl bg-brand-surface shadow-2xl sm:rounded-2xl"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between gap-4 border-b border-brand-border px-5 py-4">
          <p className="text-sm font-bold text-brand-blue">تفاصيل المنتج</p>
          <button
            type="button"
            onClick={onClose}
            aria-label="إغلاق"
            className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-brand-border text-brand-muted transition hover:border-brand-blue hover:bg-brand-sky hover:text-brand-blue focus-visible:ring-4 focus-visible:ring-brand-blue/15"
          >
        ...
        <div className="grid gap-6 p-5 sm:p-7 md:grid-cols-2">
          ...
          <div className="flex flex-col gap-4">
            ...
            <h2 className="text-2xl font-extrabold leading-9 text-brand-ink sm:text-3xl">
              {product.name}
            </h2>
            ...
            <div className="rounded-2xl bg-brand-sky p-4 text-sm leading-7 text-brand-navy">
