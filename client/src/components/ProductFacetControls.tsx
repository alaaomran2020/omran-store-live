...
export function ProductFacetControls({
  ...
  return (
    <section className="rounded-2xl border border-brand-border bg-white p-3.5 shadow-sm sm:p-4" aria-labelledby="advanced-filters-title">
      ...
      {activeFilters.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-2" aria-label="الفلاتر النشطة">
          {activeFilters.map(filter => (
            <button key={filter.key} type="button" onClick={filter.onClear} className={`inline-flex min-h-9 items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-extrabold transition ${isPopup ? "border-[#e4d3ee] bg-[#f9f2fc] text-[#6b278f] hover:bg-[#f3e6f8]" : "border-brand-border bg-brand-sky/50 text-brand-navy hover:bg-brand-sky"}`} aria-label={`إلغاء فلتر ${filter.label}`}>
