...
export default function Products({ catalog = "toys" }: ProductsProps) {
  ...
  const products = useMemo(
    () => filterProductsByCatalog(sourceProducts, catalog),
  ...
  const visibleProducts = useMemo(() => {
    ...
    const byAge = isPopup || age === ALL ? byCategory : filterProductsByAge(byCategory, age);
    return searchProducts(byAge, search) as Product[];
  }, [products, search, category, age, isPopup]);

  useEffect(() => {
    const term = search.trim();
    if (term.length < 2) return;
    const timer = setTimeout(
      () => trackEvent("product_search", { term, results: visibleProducts.length, catalog }),
      600
    );
  ...
  const categoryChip = (value: string, label: string) => (
    <button
      ...
      className={`min-h-11 shrink-0 snap-start rounded-full px-4 py-2 text-sm font-bold transition focus-visible:outline-none focus-visible:ring-4 ${
        category === value
          ? isPopup
            ? "bg-[#5e2181] text-white shadow"
            : "bg-brand-navy text-white shadow"
          : isPopup
            ? "border border-[#e8d8f1] bg-white text-[#5e2181] hover:border-[#8a3aaa]"
    ...
    <div dir="rtl" className="min-h-screen bg-brand-cream text-brand-ink">
      <header className="sticky top-0 z-40 border-b border-brand-border/80 bg-brand-cream/95 backdrop-blur">
        <div className="container flex min-h-16 items-center justify-between gap-3 sm:min-h-20 sm:gap-4">
          ...
          <nav className="flex shrink-0 items-center gap-1 text-xs font-bold text-brand-muted sm:gap-3 sm:text-sm">
            <a href="/products" className={`inline-flex min-h-11 items-center rounded-xl px-3 transition hover:bg-brand-sky hover:text-brand-blue ${!isPopup ? "text-brand-blue" : ""}`}>
              لعب الأطفال
            </a>
            <a href="/popup" className={`inline-flex min-h-11 items-center rounded-xl px-3 transition hover:bg-[#f7effb] hover:text-[#6b278f] ${isPopup ? "text-[#6b278f]" : ""}`}>
              POP UP
            </a>
          </nav>
        </div>
      ...
      <main>
        <section className={`container grid gap-5 py-8 sm:gap-8 sm:py-12 lg:grid-cols-[1.1fr_.9fr] lg:items-end lg:py-20 ${isPopup ? "relative" : ""}`}>
          <div>
            <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-bold ${isPopup ? "bg-[#f6e8fb] text-[#6b278f]" : "bg-brand-yellow/25 text-brand-navy"}`}>
              <Sparkles size={15} /> {isPopup ? "POP UP – Gifts & Balloons" : "كتالوج لعب أطفال موثّق"}
            </span>
            <h1 className={`mt-4 max-w-3xl text-[2rem] font-extrabold leading-[1.16] sm:mt-5 sm:text-5xl lg:text-6xl ${isPopup ? "text-[#4f1b68]" : "text-brand-navy"}`}>
              {isPopup ? (
                <>هدايا، بالونات ومستلزمات حفلات <span className="text-[#8a3aaa]">POP UP</span></>
              ) : (
                <>اكتشف لعب الأطفال من <span className="text-brand-blue">شركة عمران التجارية</span></>
              )}
            </h1>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-brand-muted sm:mt-5 sm:text-lg sm:leading-8">
              {isPopup
                ? "واجهة مستقلة لمنتجات POP UP المعتمدة فقط. لا تظهر هنا ألعاب الأطفال العامة، وللسعر والتوفر تواصل معنا عبر واتساب."
                : "شاهد ألعاب الأطفال المعتمدة وصورها وتفاصيلها، واختار حسب السن لما تكون بيانات العمر موثقة، وللسعر والتوفر تواصل معنا عبر واتساب."}
            </p>
            <div className="mt-5 sm:mt-6">
              <button
                type="button"
                onClick={handleShare}
                className={`inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-bold text-white shadow-md transition active:scale-[0.99] focus-visible:outline-none focus-visible:ring-4 sm:w-auto sm:rounded-full sm:shadow-lg ${isPopup ? "bg-[#6b278f] hover:bg-[#572073] focus-visible:ring-[#6b278f]/20" : "bg-brand-blue hover:bg-brand-blue-hover focus-visible:ring-brand-blue/20"}`}
              >
                <Share2 size={18} aria-hidden="true" /> مشاركة المنتجات
              </button>
              <p aria-live="polite" className={`mt-2 min-h-5 text-xs font-semibold sm:mt-3 sm:text-sm ${isPopup ? "text-[#6b278f]" : "text-brand-blue"}`}>
          ...
          <div className={`rounded-2xl p-5 text-white shadow-lg sm:rounded-[2rem] sm:p-7 sm:shadow-xl ${isPopup ? "bg-[linear-gradient(135deg,#35134f,#6b278f_58%,#9c4caa)]" : "bg-brand-navy"}`}>
            ...
            <p className="mt-2 text-base font-bold leading-7 sm:mt-3 sm:text-xl sm:leading-8">
              {isPopup
                ? "الكتالوج هنا يعتمد على تصنيف المنتج نفسه: بالونات، هدايا أو مستلزمات حفلات. أي منتج غير مصنف بهذه الفئات لا يدخل POP UP."
                : "فلترة العمر تستخدم فقط العمر الأدنى والأقصى الموثقين. أي منتج بدون بيانات عمر مؤكدة يظل ظاهرًا في كتالوج الألعاب العام فقط."}
            </p>
          </div>
        </section>
              ...
              <div className="mb-5 space-y-3 sm:mb-8 sm:space-y-4">
                <label className="relative block w-full">
                  ...
                  <input
                    ...
                    className={`min-h-12 w-full rounded-xl border bg-white py-3 pl-4 pr-11 text-base font-semibold text-brand-ink outline-none transition placeholder:text-brand-muted focus:ring-4 sm:max-w-md sm:rounded-full sm:text-sm ${isPopup ? "border-[#e4d3ee] focus:border-[#8a3aaa] focus:ring-[#8a3aaa]/15" : "border-brand-border focus:border-brand-blue focus:ring-brand-blue/15"}`}
                  />
                </label>
            ...
            {productsQuery.isLoading ? (
              ...
              <div className="space-y-8 sm:space-y-10">
                <div className={`rounded-2xl border p-6 text-center sm:rounded-[2rem] sm:p-10 ${isPopup ? "border-[#e4d3ee] bg-white" : "border-brand-border bg-brand-cream"}`}>
                  <span className={`inline-flex rounded-2xl p-4 ${isPopup ? "bg-[#f6e8fb] text-[#6b278f]" : "bg-brand-yellow/25 text-brand-navy"}`}>
                    <Sparkles size={30} aria-hidden="true" />
                  </span>
                  <p className={`mt-5 text-xl font-extrabold sm:text-2xl ${isPopup ? "text-[#4f1b68]" : "text-brand-navy"}`}>
                    {sourceError || productsQuery.isError
                      ? "المنتجات غير متاحة للعرض حاليًا"
                      : isPopup && sourceProducts.length > 0
                        ? "لا توجد منتجات POP UP منشورة حاليًا"
                        : "المتجر قيد التجهيز — المنتجات قادمة قريبًا"}
                  </p>
                  <p className="mx-auto mt-3 max-w-lg text-sm leading-7 text-brand-muted">
                    {sourceError || productsQuery.isError
                      ? "نعمل على تحديث الكتالوج الآن. جرّب التحديث أو تواصل معنا عبر واتساب."
                      : isPopup && sourceProducts.length > 0
                        ? "سيظهر هنا فقط أي منتج يتم تصنيفه صراحة ضمن بالونات أو هدايا أو مستلزمات حفلات؛ ألعاب الأطفال العامة مستبعدة بالكامل."
                        : notConfigured
                          ? "سنضيف أول المنتجات بعد اعتماد بياناتها وصورها."
                          : "بمجرد إضافة أول منتج معتمد سيظهر هنا تلقائيًا."}
                  </p>
                  <button
                    type="button"
                    onClick={() => productsQuery.refetch()}
                    className={`mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl px-6 py-3 text-sm font-bold text-white transition sm:w-auto sm:rounded-full ${isPopup ? "bg-[#6b278f] hover:bg-[#572073]" : "bg-brand-blue hover:bg-brand-blue-hover"}`}
                  >
              ...
              <>
                ...
                {visibleProducts.length === 0 ? (
                  <div className={`rounded-2xl border p-6 text-center sm:rounded-[2rem] sm:p-10 ${isPopup ? "border-[#e4d3ee] bg-white" : "border-brand-border bg-brand-cream"}`}>
                    <p className={`text-base font-extrabold sm:text-lg ${isPopup ? "text-[#4f1b68]" : "text-brand-navy"}`}>
                      {!isPopup && age !== ALL ? "لا توجد منتجات ببيانات عمر موثقة تطابق الفئة دي" : "لا توجد نتائج مطابقة لبحثك"}
                    </p>
                    ...
                    <button
                      ...
                      className={`mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-xl border px-5 py-2 text-sm font-bold transition sm:w-auto sm:rounded-full ${isPopup ? "border-[#e4d3ee] text-[#6b278f] hover:border-[#8a3aaa] hover:bg-[#f7effb]" : "border-brand-border text-brand-blue hover:border-brand-blue hover:bg-brand-blue/5"}`}
                    >
                      مسح البحث والفلاتر
                    </button>
                  </div>
          ...
          <div className="container flex flex-col items-stretch gap-5 text-center sm:flex-row sm:items-center sm:justify-between sm:text-right">
            <div>
              ...
              <p className="mt-2 max-w-md text-sm leading-7 text-white/70">
                {isPopup
                  ? "POP UP – Gifts & Balloons — هدايا، بالونات ومستلزمات حفلات ضمن شركة عمران التجارية."
                  : "لعب أطفال — صور وتفاصيل تساعدك تختار، والاستفسار مباشرة عبر واتساب."}
              </p>
            </div>
            {storeWhatsAppUrl && (
              <a
